...
class DataCoachController {
  static async getCoachNotifications(
    req: Request,
    res: Response
  ) {
    try {
      ...
      const response =
        await service.getCoachNotifications(
          Number(gymId)
        );
  ...
  static async getUsersList(
    ...
    try {
      const gymId =
        req.params.gymId || req.headers.gym_id;
  ...
  static async deleteStudent(
    ...
    try {
      const gymId =
        req.params.gymId || req.headers.gym_id;
      const deleteStudentId =
        req.params.deleteStudentId;
  ...
  static async updateCoachInfos(
    ...
    try {
      ...
      const response =
        await service.updateCoachInfos(
          id,
          birthday,
          email,
          name,
  ...
  static async getTrainings(
    ...
    try {
      const gymId =
        req.params.gymId || req.headers.gym_id;
      checkMissingField(gymId, 'gymId');
      const response = await service.getTrainings(
        Number(gymId)
      );
  ...
  static async putTraining(
    ...
    try {
      ...
      const body = req.body;
      checkMissingField(gymId, 'gymId');
      const response = await service.putTraining(
        Number(gymId),
        body
      );
  ...
  static async getExercieis(
    ...
    try {
      const gymId =
        req.params.gymId || req.headers.gym_id;
      checkMissingField(gymId, 'gymId');
      const response = await service.getExercieis(
        Number(gymId)
      );
      return res.status(200).json(response);
    } catch (error: any) {
  ...
  static async getExercise(
    ...
    try {
      ...
      const response = await service.getExercise(
        Number(exerciseId),
        Number(gymId)
      );
      return res.status(200).json(response);
  ...
  static async divisionList(
    ...
    try {
      const gymId =
        req.params.gymId || req.headers.gym_id;
      checkMissingField(gymId, 'gymId');
      const response = await service.divisionList(
        Number(gymId)
      );
  ...
  static async getTraining(
    ...
    try {
      ...
      const response = await service.getTraining(
        Number(gymId),
        Number(trainingId)
      );
  ...
  static async getTrainingDivisionOptions(
    req: Request,
    res: Response
  ) {
    try {
      ...
      checkMissingField(gymId, 'gymId');
      const response =
        await service.getTrainingDivisionOptions(
          Number(gymId)
        );
